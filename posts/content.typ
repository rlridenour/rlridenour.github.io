// content.typ
// Slide content for Beamer-Style Typst Slides Using Org Mode, written once as a function of its building
...
Org Mode's LaTeX Beamer support made producing lecture slides incredibly quick and easy. Eventually, I developed a workflow by which I could produce Beamer slides and a handout that contained more detailed explanations not found on the slides all from the same source document. I wrote my own Beamer theme called #link("https://github.com/rlridenour/basic")[basic], replicating the Keynote Basic White theme. It's a very simple theme that has white, gray, and black variations and a title slide with room for the university's logo. It works well, and gives me everything I need. The only downside is compilation time. A large presentation that I used this week compiled in twelve seconds for the slides, another seventeen seconds for the handout. Then, as it often goes, I immediately notice a mistake, fix, recompile, repeat....

I played with #link("https://typst.app")[Typst] when it first came out. I was impressed, but it wasn't able to do then what I needed. It's now become a robust publishing system with a package system that provides powerful functionality, including a very good presentation package, #link("https://typst.app/universe/package/touying")[Touying]. Typst text markup syntax is similar to Org syntax in complexity, and It compiles even complex documents instantly. So, I wondered if I could replicate my slide/handout workflow in Typst.
...
- a custom Org export backend. It turns the Org
...
The code is on #link("https://github.com/rlridenour/touying-basic-theme")[Github].

Tagged: #link("file:../tags/emacs.org")[Emacs] #link("file:../tags/org.org")[Org]  #link("file:../tags/typst.org")[Typst]
...
[1] Accessibility has, for very good reasons, become very important. We obviously want all of our products to be as accessible as possible to all of our students. Typst has an excellent #link("https://typst.app/docs/guides/accessibility/")[accessibility guide] to help in that endeavor.
]
